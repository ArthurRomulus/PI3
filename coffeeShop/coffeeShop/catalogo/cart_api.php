...
/** Normaliza la ruta de imagen a ../../Images/ (I mayúscula) salvo que sea URL absoluta */
function resolve_img($raw) {
  $s = trim((string)($raw ?? ''));
  if ($s === '') return '../../Images/placeholder.png';
  if (preg_match('~^https?://~i', $s)) return $s;

  // si ya viene normalizada correctamente, la respetamos
  if (strpos($s, '../../Images/') === 0) return $s;

  // normaliza slashes y limpia ./ y ../
  $s = preg_replace('~\\\\+~', '/', $s);
  $s = preg_replace('~^(\./|(\.\./)+)~', '', $s);

  // quita prefijos comunes, indiferente a mayúsc/minúsc.
  // (assets, images, Images, etc.)
  $s = preg_replace('~^(assest|assets?|images?)\/~i', '', $s);
...
switch ($action) {
  ...
  case 'list': {
    ...
    foreach ($_SESSION['cart'] as $it) {
      ...
      $items[] = [
        ...
        'precio'   => $precioUnit,
        'qty'      => $qty,
        'foto'     => resolve_img($it['foto'] ?? null), // <- asegura ../../Images/
        'subtotal' => $sub,
        'sabor'    => $it['sabor'] ?? null,
  ...
  case 'add': {
    ...
    $precio   = isset($_POST['precio']) ? (float)$_POST['precio'] : 0.0; // unitario con extras
    ...
    if ($precio <= 0 || $nombre === '' || $foto === null) {
      ...
      $foto   = $foto !== null ? $foto : $info['foto']; // ya viene normalizada a ../../Images/
      $sabor  = $info['sabor'];
      $tamano = $info['tamano'];
    } else {
      // normaliza lo que llegue del front
